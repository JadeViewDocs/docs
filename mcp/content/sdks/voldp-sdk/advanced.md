...
```wsv
// 1. 从字节集加载（也可用 加载视窗文件资源 直接读内嵌资源，实现单 exe 全内置）
变量 结果 <类型 = 整数>
...
// 2. 根目录留空 = 服务已加载的内存包，返回 JADE://<应用标识>
变量 地址 <类型 = 文本型>
地址 = Jade.置协议服务目录 ("")
```

> 注：上游 v2.4.0 已删除 SetPublicKey（公钥注入机制废弃），JAPK **仅支持签名包**——打包时签名，加载时自动严格验签，混淆包（JPKBIN02）不再支持。
