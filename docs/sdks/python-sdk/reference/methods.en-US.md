...
app.initialize(
    ...
    app_name=None,           # Application name (JadeView 2.x)
    app_signature=None,      # App signature; placeholder must be at least 6 chars (JadeView 2.x)
    single_instance=False,   # Single-instance mode (JadeView 2.x)
)
```
...
| app_name | str \| None | Application name; generated by the SDK when omitted |
| app_signature | str \| None | Application signature; JadeView 2.x requires a non-empty value with at least 6 trimmed characters, and the SDK fills a safe fallback |
| single_instance | bool | Whether to enable single-instance mode |
...
| frame_style | str \| None | None | JadeView 2.x frame style: `normal` / `no-titlebar` / `borderless` / `title-overlay` |
| auto_save_state | bool | False | Save and restore window state automatically |
| skip_taskbar | bool | False | v2.3: keep the window out of the taskbar / Alt-Tab |
| no_activate | bool | False | v2.3: do not steal focus when shown or clicked |
...
Sets whether the window is hidden from the taskbar / Alt-Tab. Requires JadeView 2.3+.
...
Sets whether the window avoids stealing focus when shown or clicked. Requires JadeView 2.3+.
...
Sets the window level. Requires JadeView 2.3+.
...
| Parameter | Type | Description |
|------|------|------|
| level | str | `topmost`, `normal`, `bottom`, or `desktop` |
...
Gets the native HWND. Starting with JadeView 2.3, regular windows created via `create_webview_window` can also return a valid handle.
...
Gets the JadeView window id from a native HWND. Requires JadeView 2.3+.
...
| `@window.on("drag-drop")` | `(data: dict)` | v2.x drag/drop lifecycle event |
...
def on_drag_drop(data):
    ...
    # Starting with JadeView 2.3, returning True or a non-empty string in
    # enter/drop can synchronously intercept or consume the operation.
    return False
```
...
System clipboard text API. Requires JadeView 2.x.
...
| Method | Description |
|------|------|
| `Clipboard.write_text(text)` | Writes text; returns `True` on success |
| `Clipboard.read_text(buffer_size=65536)` | Reads text; returns `None` when empty or failed |

---

## System

System information and v2.3 system integration APIs.
...
| Method | Description |
|------|------|
| `System.is_windows_11()` | Whether the current system is Windows 11 |
| `System.locale()` | System locale, such as `en-US` |
| `System.displays()` | Display information list |
| `System.cursor_position()` | Current cursor position |
| `System.get_path(name)` | Gets a system path such as `desktop`, `documents`, `appdata`, or `temp` |
| `System.webview_version()` | WebView2 Runtime version |
...
YAML persistence API. Requires JadeView 2.3+; use it after `app.on_ready`.
...
| Method | Description |
|------|------|
| `Storage.set(file_name, key_path, value)` | Writes a value; non-string values are serialized as JSON |
| `Storage.set_str(file_name, key_path, value)` | Writes a literal string |
| `Storage.get(file_name, key_path, default=None)` | Reads a path value |
| `Storage.get_str(file_name, key_path, default=None)` | Reads a value as a literal string (pairs with `set_str`; keeps `"123"` a string) |
| `Storage.get_all(file_name, default=None)` | Reads the whole YAML file |
| `Storage.has(file_name, key_path)` | Checks whether a path exists |
| `Storage.delete(file_name, key_path)` | Deletes a path |
| `Storage.clear(file_name)` | Clears the file to `{}` |
| `Storage.delete_file(file_name)` | Deletes the YAML file |
| `Storage.keys(file_name, key_path="")` | Lists keys under a path |
| `Storage.length(file_name, key_path="")` | Returns object key count or array length |

---

## Tray

System tray API. Requires JadeView 2.x and is usually created after `app.on_ready`.
...
| Method | Description |
|------|------|
| `set_tooltip(text)` | Sets the tooltip |
| `set_icon(icon_path)` | Sets the tray icon from a file |
| `set_icon_from_bytes(data)` | Sets the tray icon from bytes |
| `set_menu(items)` | Sets the context menu |
| `on_click(callback)` | Listens for tray click events |
| `show()` / `hide()` | Shows or hides the tray icon |
| `destroy()` | Destroys the tray icon |

---

## HotKey

Global hotkey API. Requires JadeView 2.x.
...
| Method | Description |
|------|------|
| `HotKey.register(combo, callback=None)` | Registers a global hotkey; can be used as a decorator |
| `HotKey.unregister(hotkey_id)` | Unregisters a hotkey |
| `HotKey.unregister_all()` | Unregisters all hotkeys |
...
:::info{title="v2.3 DLL Rule"}
`get_architecture()` returns `x86`, `x64`, or `arm64` based on the current Python interpreter architecture. The downloader only selects the latest build inside the SDK-adapted release tag and never automatically crosses compatibility boundaries such as `2.2 -> 2.3`.
:::
