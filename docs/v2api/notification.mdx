...
| `button1` / `button2` | 通知上的按钮文字；用户点击会触发 **`notification-action`** 等（见 [事件类型](/v2api/event-types#notification-events)）。 |
| `text3` | 多一行辅助文字。 |
...
不再需要单独调「注册应用」：**第一次显示通知时库会在内部完成注册**。用户点击、关闭或显示失败时，宿主通过 **`jade_on`** 收到的事件名见 [事件类型 · 通知相关事件](/v2api/event-types#notification-events)。
