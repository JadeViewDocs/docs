...
- `webview_settings` `WebViewSettings*` (可选) - 网页行为配置，可传 `NULL` 使用默认值
  ...
  **WebViewSettings 选项：**
  ...
  - `autofill` `int32_t` - 是否启用账号/密码自动填充。`0` = 禁用，`1` = 启用。*(2.2 新增)*
  - `general_autofill_enabled` `int32_t` - 是否启用通用表单自动填充（姓名/地址/电话等）。`0` = 禁用，`1` = 启用。*(2.2 新增)*
  - `incognito` `int32_t` - 是否以无痕/隐私浏览模式运行。`0` = 正常模式，`1` = 无痕模式。开启后页面渲染会变慢。*(2.2 新增)*
  - `disable_clipboard` `int32_t` - 是否禁用剪贴板读写权限。`0` = 允许，`1` = 禁用。*(2.2 新增)*
  - `proxy_url` `string` - 代理URL，支持 HTTP 和 SOCKS5 代理（如 `"http://127.0.0.1:7890"` 或 `"socks5://127.0.0.1:1080"`）。`NULL` 表示不使用代理。*(2.2 新增)*
  - `focused` `int32_t` - WebView 初始是否自动获取焦点。`0` = 不获取焦点，`1` = 自动聚焦（默认 `1`）。*(2.2 新增)*
