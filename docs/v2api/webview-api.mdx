...
所有操作都针对 **`window_id`**（没有单独的 webview 句柄）。`WebViewSettings` 在 [核心 API](/v2api/index#webview-settings) 里按字段说明过，这里侧重**创建之后**还能调什么。
...
**用途**：在**页面环境里跑一段 JS**。返回值只表示任务有没有排队成功；真正的脚本返回值通过 **`javascript-result`** 等事件回来（见 [事件类型](/v2api/event-types#javascript-result)）。
