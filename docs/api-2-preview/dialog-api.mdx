...
void example_sync_dialog() {
  ...
  params.filters = R"([{"name":"图片","extensions":["jpg","png"]}])";
...
| 字段 | 说明 | 示例 |
|------|------|------|
| `window_id` | **父窗口 ID**，对话框会作为该窗口的模态对话框弹出。传 `0` 表示没有特定父窗口 | `1` |
| `title` | **对话框标题栏文字**，传 `NULL` 或空字符串使用默认标题 | `"选择图片"` |
| `default_path` | **默认打开的路径**，对话框打开时会定位到这个目录。传 `NULL` 或空字符串使用系统默认位置 | `"D:/Documents"` |
| `button_label` | **确认按钮的文字**，只在部分系统上生效。传 `NULL` 或空字符串使用默认文字 | `"导入"` |
| `filters` | **文件类型筛选规则**，JSON 数组字符串，定义可以选择哪些类型的文件 | 见下方示例 |
| `properties` | **对话框行为选项**，用逗号分隔的特性名字符串，控制对话框的功能 | 见下方示例 |
...
| 参数 | 说明 |
|------|------|
| `params` | 指向 `FileDialogParams` 结构体的指针，配置对话框选项 |
...
| 字段 | 说明 |
|------|------|
| `window_id` | 父窗口 ID，传 `0` 表示没有特定父窗口 |
| `title` | 对话框标题栏文字 |
| `message` | 主提示文字，显示在对话框上方 |
| `detail` | 补充说明文字，显示在主提示下方，可以是多行 |
| `buttons` | 按钮列表，用 `\|` 分隔多个按钮名，如 `"确定\|取消"` |
| `default_id` | 默认聚焦的按钮索引，从 0 开始 |
| `cancel_id` | 按 Esc 键或点击关闭按钮时触发的按钮索引，从 0 开始 |
| `type_` | 对话框类型，控制图标风格 |
...
|------|------|
| `params` | 对话框参数结构体指针 |
| `callback` | 异步回调，函数签名为 `void (JADEVIEW_CALL *)(const char* json_result)`，参数是 UTF-8 JSON 结果 |
