...
## `WebViewWindowOptions`（创建窗口时） {#webview-window-options}
...
| `frame_style` | 要系统边框+标题栏、只要边框不要标题栏、还是完全无边框（`normal` / `no-titlebar` / `borderless`）。 |
...
## `WebViewSettings`（页面行为） {#webview-settings}
