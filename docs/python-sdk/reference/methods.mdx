...
使用 `window.on(event, callback)` 方法监听事件。推荐使用 `Events` 常量。

#### 窗口事件

| 事件常量 | 事件名 | 回调参数 | 描述 |
|----------|--------|----------|------|
| `Events.WINDOW_CREATED` | window-created | json_data | 窗口创建完成 |
| `Events.WINDOW_CLOSING` | window-closing | json_data | 窗口即将关闭（返回 1 可阻止） |
| `Events.WINDOW_CLOSED` | window-closed | json_data | 窗口已关闭 |
| `Events.WINDOW_DESTROYED` | window-destroyed | json_data | 窗口已销毁 |
| `Events.WINDOW_FOCUSED` | window-focused | json_data | 获得焦点 |
| `Events.WINDOW_BLURRED` | window-blurred | json_data | 失去焦点 |
| `Events.WINDOW_RESIZED` | window-resized | json_data | 大小改变，数据：`{"width": 宽度, "height": 高度}` |
| `Events.WINDOW_MOVED` | window-moved | json_data | 位置改变，数据：`{"x": x, "y": y}` |
| `Events.WINDOW_STATE_CHANGED` | window-state-changed | json_data | 状态改变，数据：`{"isMaximized": 布尔值}` |
| `Events.FILE_DROP` | file-drop | files, x, y | 文件拖放（参数已自动解析） |

#### WebView 事件

| 事件常量 | 事件名 | 回调参数 | 描述 |
|----------|--------|----------|------|
| `Events.WEBVIEW_WILL_NAVIGATE` | webview-will-navigate | json_data | 即将导航 |
| `Events.WEBVIEW_DID_START_LOADING` | webview-did-start-loading | json_data | 开始加载 |
| `Events.WEBVIEW_DID_FINISH_LOAD` | webview-did-finish-load | json_data | 加载完成 |
| `Events.WEBVIEW_NEW_WINDOW` | webview-new-window | json_data | 新窗口请求 |
| `Events.WEBVIEW_PAGE_TITLE_UPDATED` | webview-page-title-updated | json_data | 标题更新 |
| `Events.FAVICON_UPDATED` | favicon-updated | json_data | 图标更新 |

#### 其他事件

| 事件常量 | 事件名 | 回调参数 | 描述 |
|----------|--------|----------|------|
| `Events.THEME_CHANGED` | theme-changed | json_data | 主题改变 |
| `Events.JAVASCRIPT_RESULT` | javascript-result | json_data | JS 执行结果 |
| - | created | Window | 内部事件：窗口创建完成 |
| - | closed | - | 内部事件：窗口已关闭 |
| - | page-loaded | url, status | 内部事件：页面加载完成 |
...
# 使用 Events 常量监听窗口大小改变
@window.on(Events.WINDOW_RESIZED)
def on_resize(json_data):
    data = json.loads(json_data)
    print(f"窗口大小改变: {data['width']} x {data['height']}")
