...
window.create_webview_window(
    url,
    frame_style="no-titlebar",    # 无标题栏（有边框）
    # 或 frame_style="borderless"    # 完全无边框
    # 或 frame_style="title-overlay" # 无标题栏 + 系统按钮覆盖层（Windows 专属）
)
```

### title-overlay 和 no-titlebar 有什么区别？

- `no-titlebar`：去掉标题栏，需要自己用 HTML/CSS 实现窗口控制按钮（最小化、最大化、关闭）
- `title-overlay`：去掉标题栏，但系统自动在右上角提供原生的最小化/最大化/关闭按钮，无需自行实现
