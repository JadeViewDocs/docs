...
window.create_webview_window(
    ...
    *,
    ...
    frame_style: str | None = None,    # 边框样式："normal"/"no-titlebar"/"borderless"/"title-overlay"
...
| 函数 | 说明 |
|------|------|
| `set_window_frame_style(window_id, frame_style)` | 运行时修改边框样式："normal"/"no-titlebar"/"borderless"/"title-overlay" |
| `set_titlebar_overlay_style(window_id, height, icon_color_hex, hover_bg_hex)` | 自定义 title-overlay 按钮覆盖层外观 |
| `jade_print(window_id)` | 打印 WebView 内容（打开系统打印对话框） |
...
:::tip
打印功能 `jade_print(window_id)` 位于 `jadeview.window` 模块中。
:::
...
- **`.japk` 文件路径**：指向 JAPK 资源包文件（JadeView 直接从包内读取，无需解压）
