...
您可以在窗口创建后动态切换边框样式，无需重新创建窗口：

```python
from jadeview import window

# 切换到 title-overlay 模式（Windows 专属）
...
可选值：
- `"normal"` — 系统标准边框 + 标题栏
- `"no-titlebar"` — 有边框但无标题栏
- `"borderless"` — 完全无边框无标题栏
- `"title-overlay"` — 有边框 + 无标题栏 + 右上角内置最小化/最大化/关闭按钮（Windows 专属）

### Title Overlay 模式

`title-overlay` 是 Windows 专属的边框样式，提供系统原生的窗口控制按钮（最小化、最大化、关闭），同时允许网页内容延伸到标题栏区域。适合需要自定义标题栏外观但不想自己实现窗口控制按钮的场景。
...
window.set_titlebar_overlay_style(
    win_id,
    height=40,                  # 按钮区域高度（像素）
    icon_color_hex="#333333",    # 按钮图标颜色
    hover_bg_hex="#00000020",    # 悬浮背景色（支持 #RRGGBBAA 透明度）
)
```

:::info
关闭按钮的悬浮背景色固定为红色（`#E81123`），图标固定为白色，不受 `set_titlebar_overlay_style` 影响。
:::

### 前端配合

在 `title-overlay` 模式下，网页需要设置拖动区域让用户可以拖动窗口：

```html
<div class="titlebar" style="-webkit-app-region: drag; height: 40px;">
    <span>我的应用</span>
</div>
...
JadeView 提供内置的协议服务，将本地目录或 JAPK 资源包映射为可访问的 URL：
...
def on_ready(window_id, data):
    ...
    # 或加载 JAPK 资源包（无需解压）
    # base_url = tools.set_protocol_service_path("C:/myapp/app.japk")
