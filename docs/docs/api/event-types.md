...
本页列出主进程通过 **`jade_on`** 订阅后，库可能派发的事件：**`event_data` 一般是 UTF-8 的 JSON 字符串**（少数为纯文本，如 `app-ready` 成功时的 `success`）。`jade_on` 回调的第一个参数 **`window_id`** 标明是哪个窗口（与具体事件无关的全局事件多为 `0`，以各节为准）。
...
主进程判断方式：`window_id === 1` 表示成功，`window_id === 0` 表示失败。
