...
:::caution 槽位上限
`On` 与 `RegisterIPCHandler` **共享** `MaxEventHandlers` = 64 个槽位；IPC handler 无注销 API（上游头文件亦无），注册后**永久占用**一个槽位，规划通道数量时留意。
:::
...
:::caution Linux 托盘须先探测
在没有 StatusNotifier 托盘协议的桌面（如 Debian/GNOME 默认桌面）上，beta.10 的 `TrayCreate` 会让库 GUI 线程直接崩溃而非返回 0。调用前先探测会话 D-Bus 上有无 `org.kde.StatusNotifierWatcher`，详见[高级用法](./advanced#linux-托盘协议探测)。
:::
