...
| `background_throttling` | int | 背景限速策略，0=默认（启用背景限速），1=禁用背景限速。背景限速是浏览器的性能优化机制，当页面在后台时会降低 JavaScript 执行速度，某些场景下（如后台持续运行计算）需要禁用此功能。 |
...
|------|------|------|
| `window_id` | uint32_t | 窗口 ID |
| `url` | const char* | 页面 URL |
| `status` | const char* | 加载状态 |
...
| `mime_type` | const char* | 文件 MIME 类型 |
| `x` | double | 拖放 X 坐标 |
| `y` | double | 拖放 Y 坐标 |
